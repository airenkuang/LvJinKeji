...
public class SysManager_CHange : MonoBehaviour
{
    ...
    public static SysManager_CHange _instance;

    private MediaPlayerEvent.EventType firstFrameReady;
    ...
    private void Awake()
    {
        ...
        firstFrameReady = MediaPlayerEvent.EventType.FirstFrameReady;
    ...
    void Start()
    {
        foreach (var btn in btns)
        {
            ...
            moveCompleted += delegate (DisplayUGUI dis)
            {
                ...
                dis.transform.DOScale(0, .5f).SetEase(Ease.Linear);//.OnComplete(() =>
               // {
                    //Debug.Log("completed");
    ...
    void OnBtnClick(Button btn)
    {
        ...
        if (btn.tag == "left")
        {
            mp = displayLeft.GetComponent<DisplayUGUI>()._mediaPlayer;
            mp.OpenVideoFromFile(MediaPlayer.FileLocation.RelativeToStreamingAssetsFolder, videoPath, true);
            displayLeft.GetComponent<VideoController>().playIcon.SetActive(false);
            //displayLeft.GetComponent<VideoController>().Play();
            //displayLeft._defaultTexture = tex_show;
            mp.Events.AddListener((mp, firstFrameReady, errorCode) => {
                displayLeft.transform.DOScale(scaleFactor, .5f).SetEase(Ease.Linear);
            });
        }
        else
        {
             mp = displayRight.GetComponent<DisplayUGUI>()._mediaPlayer;
            mp.OpenVideoFromFile(MediaPlayer.FileLocation.RelativeToStreamingAssetsFolder, videoPath, true);
            displayRight.GetComponent<VideoController>().playIcon.SetActive(false);
            //displayRight.GetComponent<VideoController>().Play();
            // displayRight._defaultTexture = tex_show;
            mp.Events.AddListener((mp, firstFrameReady, errorCode)=>{
                displayRight.transform.DOScale(scaleFactor, .5f).SetEase(Ease.Linear);
            });
        }
